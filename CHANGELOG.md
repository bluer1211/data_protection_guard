...
本文件記錄 Data Protection Guard Guard Plugin 的所有版本變更。

## [1.0.7] - 2025-08-20 18:00:00 CST

### 修復
- **日誌頁面連結錯誤**: 修復日誌頁面中指向已刪除設定頁面的連結錯誤
- **路由一致性**: 確保所有頁面連結都指向正確的統一設定頁面
- **錯誤處理**: 改善錯誤處理機制，提升使用者體驗

### 技術改進
- 更新日誌頁面模板中的設定頁面連結路徑
- 統一使用 `plugin_settings_path('data_protection_guard')` 作為設定頁面路徑
- 確保所有導航連結的一致性
...
### 使用者介面改善
- **統一設定頁面**: 合併 `/settings/plugin/data_protection_guard` 和 `/data_protection/settings` 為單一設定頁面
- 在設定頁面新增資料庫日誌記錄選項
- 新增自動清理天數設定欄位（1-365天）
- 新增直接連結到日誌檢視頁面的按鈕
- 改善設定頁面的佈局和使用者體驗
